...
class autoExcelAdjust():
    ...
    def edit_final_excel(self, userId, today, title, startDate, endDate):
        ...
        ship_ed = excel.Workbooks.Open(
            downloadPath + f"\\{userId}({today}).xls")
        shipped_active = ship_ed.ActiveSheet

        # 양식 열고, 입력 시작
        filename = "original2.xlsx"
        ...
        self.edit_excel(shipped_active, wb_origin_active,
        ...
        # 배송중 엑셀파일 열기
        ship_ing = excel.Workbooks.Open(
            downloadPath + f"\\{userId}({today}) (1).xls")
        shipping_active = ship_ing.ActiveSheet

        # 아까 편집한 양식 열고, 입력 시작
        wb_origin2 = excel.Workbooks.Open(save_path)
        wb_origin2_active = wb_origin2.ActiveSheet
        self.edit_excel(shipping_active, wb_origin2_active,
                        startDate_2, endDate_2)

        # 정산일자 적기
        wb_origin2_active.Range(
            "C199").Value = f"정산일자: {startDate} ~ {endDate}"
    ...
    def edit_excel(self, read_file, write_file, startDate, endDate):
        ...
        written_lines = excel.WorkSheetFunction.CountA(
            write_file.Range("B:B"), 0)
        written_lines = round(written_lines)

        self.copy_and_paste(read_file, write_file, lines,
                            startDate, endDate, written_lines)
    ...
    def copy_and_paste(self, read_file, write_file, lines, startDate, endDate, line_num):
        ...
        for line in range(2, lines+1):
            ...
            # 정산 대상에 해당하는 날짜 지목
            # 그 날짜에 해당하는 셀들만 복붙하기
            time_read = read_file.Range(f"V{line}").Value.strftime(format)
            ...
            if startDate <= time_read_file <= endDate:
                ...
                # 주문완료일자: 홈페이지 파일 V2 -> 정산자료 B3
                read_file.Range(f"V{line}").Copy()
                ...
                write_file.Range(f"H{read_line}").Select()
                write_file.Paste()

                # 수령인: 홈페이지 파일 J2 -> 정산자료 K3
                read_file.Range(f"J{line}").Copy()
                write_file.Range(f"I{read_line}").Select()
                write_file.Paste()
